...
const ShiftSwitching = () => {
  ...
  const handleRegister = async () => {
    ...
    if (window.confirm(`តើអ្នកពិតជាចង់កំណត់សិស្ស ${student.name} ជាសិស្សប្តូរវេនមែនទេ?`)) {
      try {
        ...
        alert('មានបញ្ហាក្នុងការរក្សាទុកទិន្នន័យ។ សូមពិនិត្យមើលថាតើអ្នកបាន Run SQL ដោះស្រាយ Column ថ្មីនៅក្នុង Supabase ឬនៅ? Error: ' + error.message);
      }
    }
  };
  ...
  const handleSwitchShift = async (student: Student) => {
    ...
    if (window.confirm(`តើអ្នកចង់ប្តូរសិស្ស ${student.name} ទៅកាន់ថ្នាក់ ${targetClass.name} មែនទេ?`)) {
      ...
      // Swap current class and alternate class
  ...
  // Only show students who are shift switchers
  const shiftSwitchers = students.filter(s => s.isShiftSwitching);
  ...
  // Eligible students for the dropdown (not already a switcher)
  const eligibleStudents = students.filter(s => !s.isShiftSwitching);

  // Helper to extract grade from class name (e.g. "8A1" -> "8")
  ...
  const eligibleAlternateClasses = classes.filter(c => {
    ...
    const studentClassObj = classes.find(cl => cl.id === selectedStudent.class);
    if (!studentClassObj) return false;
    
    return getGrade(c.name) === getGrade(studentClassObj.name);
  });
      ...
      <div className="bg-white border border-gray-200 shadow-sm rounded-sm mb-6">
        ...
        <div className="p-4 flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            ...
            <select 
              className="w-full bg-white border border-gray-300 text-gray-800 text-sm rounded-sm px-3 py-2 outline-none focus:border-[#48b5c9] focus:ring-1 focus:ring-[#48b5c9] transition-all"
          ...
          <button 
            className="bg-[#48b5c9] hover:bg-[#3aa3b7] text-white px-6 py-2 rounded-sm text-sm font-medium flex items-center gap-2 transition-all h-[38px] w-full md:w-auto justify-center"
            onClick={handleRegister}
          >
            <UserPlus size={16} /> បន្ថែម
          </button>
        </div>
      </div>
                ...
                <tr key={s.id} className="hover:bg-gray-50/50 transition-colors border-b border-gray-100">
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        onClick={() => handleSwitchShift(s)}
                        className="bg-orange-100 text-orange-700 hover:bg-orange-200 hover:text-orange-800 px-3 py-1.5 rounded-sm text-xs font-bold transition-colors flex items-center gap-1.5"
                      >
                        <ArrowLeftRight size={14} /> ប្តូរវេន
                      </button>
                      <button 
                        onClick={() => handleRemoveStatus(s)}
                        className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-sm transition-colors ml-2"
