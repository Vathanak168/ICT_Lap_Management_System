 ...
 * If names are identical, falls back to studentId for deterministic ordering.
 */
export const compareStudentsByKhmerName = <T extends StudentLike>(a: T, b: T): number => {
  ...
  const idA = (a.studentId ?? a.student_id ?? '').trim();
  const idB = (b.studentId ?? b.student_id ?? '').trim();
