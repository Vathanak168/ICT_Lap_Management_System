...
const Sidebar = () => {
  ...
  const renderNavItem = (item: { name: string; path: string; icon: any }) => (
    <NavLink
      ...
      to={item.path}
      className={({ isActive }) =>
        `group flex items-center gap-3 px-3 py-2 rounded-xl text-xs sm:text-[13px] transition-all duration-150 ${
          isActive
            ? 'bg-blue-50/90 text-primary font-bold shadow-2xs border border-blue-150/60'
            : 'text-secondary-text font-medium hover:bg-surface-hover hover:text-main-text'
        }`
      }
    >
      {({ isActive }) => (
        <>
          <span className={`shrink-0 transition-colors ${isActive ? 'text-primary' : 'text-secondary-text/80 group-hover:text-main-text'}`}>
            {item.icon}
          </span>
  ...
  return (
    <aside className="w-[240px] bg-surface border-r border-border flex flex-col h-full shrink-0 z-10 transition-all duration-300 print:hidden pt-3 shadow-2xs">
      <nav className="flex-1 px-2.5 space-y-1 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:rgba(148,163,184,0.3)_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-200/80 hover:[&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
      ...
      <div className="p-2.5 border-t border-border/70 shrink-0">
        <NavLink 
          to="/settings" 
          className={({ isActive }) => 
            `group flex items-center gap-3 px-3 py-2 rounded-xl text-xs sm:text-[13px] transition-all duration-150 w-full ${
              isActive 
                ? 'bg-blue-50/90 text-primary font-bold shadow-2xs border border-blue-150/60' 
                : 'text-secondary-text font-medium hover:bg-surface-hover hover:text-main-text'
            }`
